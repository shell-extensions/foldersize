...
FILE_MANAGER_NAME = os.environ.get("FOLDERSIZE_FILE_MANAGER", "Nautilus")
FILE_MANAGER_VERSIONS = {
    "Nautilus": "4.0",
    "Caja": "2.0",
    "Nemo": "3.0",
}
COLUMN_NAME = "FolderSize::size"

try:
    gi.require_version(FILE_MANAGER_NAME, FILE_MANAGER_VERSIONS[FILE_MANAGER_NAME])
except KeyError as e:
    raise RuntimeError(f"Unsupported file manager: {FILE_MANAGER_NAME}") from e

from gi.repository import GObject, GLib, Gio

FILE_MANAGER = getattr(__import__("gi.repository", fromlist=[FILE_MANAGER_NAME]), FILE_MANAGER_NAME)
...
def _languages_from_env_value(value):
    if not value:
        return None

    languages = []
    for entry in str(value).split(":"):
        primary = entry.strip().split(".")[0]
        if not primary:
            continue
...
languages = None
for key in ("LC_ALL", "LC_MESSAGES"):
    languages = _languages_from_env_value(os.environ.get(key))
    if languages:
        break

if not languages:
    languages = (_languages_from_env_value(os.environ.get("LANGUAGE"))
                 or _languages_from_env_value(os.environ.get("LANG")))
...
class FolderSize(GObject.GObject,
                 FILE_MANAGER.ColumnProvider,
                 FILE_MANAGER.InfoProvider,
                 FILE_MANAGER.MenuProvider):

    __gtype_name__ = f"FolderSize{FILE_MANAGER_NAME}Extension"
    ...
    _file_refs = {}         # { path: FILE_MANAGER.FileInfo }
    ...
    def get_file_items(self, *args):
        ...
        # Nautilus reicht files, Caja/Nemo reichen (window, files).
    ...
    def get_background_items(self, *args):
        ...
        GNOME-Shell-Quick-Settings-Toggle, jetzt da foldersize eine reine
        Dateimanager-Erweiterung ist.
        """
